...
class SajuEngine:
    ...
    def _get_historical_correction(self, dt):
        ...
        for start, end in sc.DST_PERIODS:
            if start <= ts <= end: offset -= 60; break
        return offset
    ...
    def _apply_solar_correction(self, dt_in, yG, mG):
        ...
        term_today = next((t for t in self.t_db.get(year_str, []) if t['date'] == today_str and t['isMonthChange']), None)
        if term_today:
    ...
    def _calculate_power(self, palja, me_hj):
        scores, power = {'목': 0.0, '화': 0.0, '토': 0.0, '금': 0.0, '수': 0.0}, 0
        target_indices = [(0, 11), (1, 11), (2, 11), (3, 30), (5, 15), (6, 11), (7, 11)] 
        for char in palja: scores[sc.ELEMENT_MAP[char]] += 12.5
        for idx, weight in target_indices:
            char = palja[idx]
            if sc.REL_MAP.get((me_hj, sc.E_MAP_HJ[char])) in ['비겁', '인성']: power += weight
        return scores, power
    ...
    def _get_yongsin_info(self, palja, power, me_hj_hanja):
        ...
        mb = palja[3]
        johoo = "필요 없음 (중화)"
        if mb in ['亥', '子', '丑']: johoo = "화(火) - 추운 계절이라 따뜻한 기운이 최우선입니다."
        elif mb in ['巳', '午', '未']: johoo = "수(水) - 더운 계절이라 시원한 기운이 최우선입니다."
        elif mb in ['寅', '卯', '辰']: johoo = "화(火) - 만물이 성장하도록 온기가 필요합니다."
        elif mb in ['申', '酉', '戌']: johoo = "수(水) - 건조한 계절이라 적절한 수기가 필요합니다."
    ...
    def _investigate_sinsal(self, palja, me, me_hj):
        start_y, start_d = sc.BRANCHES.find(sc.SAMHAP_START_MAP[palja[1]]), sc.BRANCHES.find(sc.SAMHAP_START_MAP[palja[5]])
        g_jis = sc.GONGMANG_MAP[self.SIXTY_GANZI.index(palja[4] + palja[5]) // 10]
        pillars = []
        for i in range(4):
            g, j, special = palja[i*2], palja[i*2+1], []
            s12_y, s12_d = sc.SINSAL_12_NAMES[(sc.BRANCHES.find(j)-start_y)%12], sc.SINSAL_12_NAMES[(sc.BRANCHES.find(j)-start_d)%12]
            special.append(s12_y)
            if s12_d != s12_y: special.append(f"{s12_d}(일)")
            if j in g_jis: special.append("공망(空亡)")
            if (g+j) in ["戊戌", "庚戌", "庚辰", "壬辰", "壬戌"]: special.append("괴강살")
            if j == sc.YANGIN_MAP.get(me): special.append("양인살")
            ...
            if j == sc.GWIMUN_MAP.get(palja[5]): special.append("귀문관살")
    ...
    def _calculate_daeun(self, dt_in, yG, mG, gender, l_term, n_term):
        ...
        daeun_num = max(1, int(round(abs(((n_term if is_fwd else l_term)['dt_obj'] - dt_in).total_seconds() / 86400) / 3.0)))
    ...
    def _calculate_daeun_scores(self, daeun_list, yongsin_info, palja):
        yong, huising = yongsin_info['eokbu_elements'].split('/'), []
        ...
        for d in daeun_list:
            ...
            score = 50 + (15 if ghj in yong else 7 if ghj in huising else -10) + (35 if jhj in yong else 18 if jhj in huising else -20)
            is_jg, offset = (jhj in yong or jhj in huising), 0
            for ns in n_stems:
                if sc.D_STEM_HAB.get(dg) == ns: offset += 5
                if sc.D_STEM_CHUNG.get(dg) == ns: offset -= (1 if is_jg else 7)
            for nb in n_branches:
                if sc.D_BRANCH_HAB.get(dj) == nb: offset += 8
                if sc.D_BRANCH_CHUNG.get(dj) == nb: offset -= (3 if is_jg else 12)
    ...
    def _analyze_wealth_and_career(self, pillars, power, yongsin_elements):
        ...
        ws = 40 + (atg.count('정재')+atg.count('편재')) * (8 if power >= 50 else 4)
        if (atg.count('식신')+atg.count('상관')) > 0 and (atg.count('정재')+atg.count('편재')) > 0: ws += 15
        if 40 <= power <= 65: ws += 10
        elif power < 30 and (atg.count('정재')+atg.count('편재')) >= 3: ws -= 15
        cs = 40 + (atg.count('정관')+atg.count('편관')) * 10 + (15 if (atg.count('정관')+atg.count('편관')) > 0 and (atg.count('정인')+atg.count('편인')) > 0 else 0)
        for s in ["천을귀인", "관귀학관", "태극귀인", "백호대살"]:
            if s in asp: cs += 10 if s == "관귀학관" else 5 if s == "백호대살" else 0; ws += 5 if s in ["천을귀인", "태극귀인"] else 0; cs += 5 if s in ["천을귀인", "태극귀인"] else 0
    ...
    def _analyze_interactions(self, palja):
        ...
        for i in range(4):
            for j in range(i+1, 4):
                ps, pb = "".join(sorted([sl[i], sl[j]])), "".join(sorted([bl[i], bl[j]]))
                if ps in sc.STEM_HAB: res["천간합"].append(sc.STEM_HAB[ps])
                if ps in sc.STEM_CHUNG: res["천간충"].append(sc.STEM_CHUNG[ps])
                if pb in sc.B_HAB_6: res["지지육합"].append(sc.B_HAB_6[pb])
                if pb in sc.B_CHUNG: res["지지충"].append(sc.B_CHUNG[pb])
                if pb in sc.B_HYUNG: res["형"].append(sc.B_HYUNG[pb])
                if pb in sc.B_PA: res["파"].append(sc.B_PA[pb])
                if pb in sc.B_HAE: res["해"].append(sc.B_HAE[pb])
                if pb in sc.B_WONJIN: res["원진"].append(sc.B_WONJIN[pb])
        for key, (val, king) in sc.B_SAMHAP.items():
            match = [c for c in key if c in bl]
            if len(set(match)) >= 3: res["지지삼합"].append(f"{val} 삼합({key})")
            elif len(set(match)) == 2 and king in bl: res["지지삼합"].append(f"{''.join([sc.B_KOR[c] for c in match])} 반합({val})")
        for key, val in sc.B_BANGHAP.items():
            match = [c for c in key if c in bl]
            if len(set(match)) >= 3: res["지지방합"].append(val)
            elif len(set(match)) == 2: res["지지방합"].append(f"{''.join([sc.B_KOR[c] for c in match])} 반합")
        g_jis = sc.GONGMANG_MAP[self.SIXTY_GANZI.index(palja[4]+palja[5]) // 10]
        for i, b in enumerate(bl):
            if b in g_jis: res["공망"].append(f"{['년지','월지','일지','시지'][i]} 공망({sc.B_KOR.get(b, b)})")
    ...
    def analyze(self, birth_str, gender, location, use_yajas_i, calendar_type="양력"):
        ...
        lng_off = int(round((sc.CITY_DATA.get(location, 126.97) - 135) * 4))
